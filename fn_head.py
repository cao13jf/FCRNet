...
def build_embedding_head(features, alpha, embedding_dim=16, name='embedding_branch'):
    ...
    with tf.variable_scope(name):
        ...
        # add parameterized softmax layer
        pow_emb = tf.pow(emb, alpha)
        pow_emb = pow_emb / tf.maximum(tf.reduce_sum(pow_emb, axis=-1, keep_dims=True), 10e-8)
