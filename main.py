...
def main(_):
    ...
    if tf_flags.phase == "train":
        ...
        with tf.Session() as sess:
            model = LocalDisNet(sess, tf_flags)
            model.restore_model(tf_flags.model_dir)
            for f_name, f_path in img_path.items():
                img = imread(f_path)
                print("Processing: ", f_path)
                segs = model.segment_from_seed([img], seed_thres=0.7, similarity_thres=0.7, resize=True, with_dist=tf_flags.dist_branch)
...
if __name__ == '__main__':
    ...
    tf.app.flags.DEFINE_string("phase", "train", "model phase: train/test/evaluation")
    ...
    tf.app.flags.DEFINE_integer("training_epoches", 300,
    ...
    # test config
    tf.app.flags.DEFINE_string("test_dir", "./test/cvppp_test", "evaluation dataset directory")
    tf.app.flags.DEFINE_string("test_res", "./test/cvppp_res", "evaluation dataset directory")
